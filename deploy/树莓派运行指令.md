...
```powershell
scp -r . pi@<pi-ip>:/opt/pm-shadow/Son-of-weather
```
...
```bash
sudo cp /opt/pm-shadow/Son-of-weather/deploy/pm_shadow.service /etc/systemd/system/pm_shadow.service
...
>
> 若使用 Clash，请以 `mixed-port` 实际值为准（你当前示例为 `7897`）。
